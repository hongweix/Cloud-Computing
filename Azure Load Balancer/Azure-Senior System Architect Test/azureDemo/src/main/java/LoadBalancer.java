...
public class LoadBalancer {
    ...
    // int[] array to store the status of the three data center, originally setting to 1 for each
    // it performs as a block, if vmblock[i] value is 0, means it is blocked, we can not do check cpu 
    // utilization on it until its value return to 1
    private static int[] vmblock = {1,1,1};
	...
	/**
     * In the senior architect test, combined both test 2 and test 3, my strategy is:
     * When cpucheckornot does not arrive 5, we just do like test 3, do the health check and add new
     * vm if needed, when cpucheckornot arrives 5, we first need to check if all 3 data center are alive,
     * if they are all alive, then we can compare cpu and parse request to the minimun cpu utilization 
     * data center, however, if the data center alive is less than 3, then it is a waste of time and resource
     * to check cpu utilization, we just keep going to the next roll.
     */
	public void start() throws Exception {
		ExecutorService executorService = Executors.newFixedThreadPool(THREAD_POOL_SIZE);
        // cpucheckornot is a counter, when it arrives to 5, we need to do a cpu utilization checking
		int cpucheckornot = 0;
		while (true) {
            // cpucheckornot add 1 each time
            cpucheckornot += 1;
            // if cpucheckornot does not arrives 5
            if(cpucheckornot != 5){
    	    		...
    	    		new Thread(new Runnable(){
    	    			...
    	    			public void run(){
    	    				...
    	    				instances[0] = new DataCenterInstance("Savior", "http://"+newvm);
                                        System.out.println("now 1st vm is " + instances[0].getUrl());
                            //reset vmstatus and vmblock to 1
    	    				vmstatus[0] = 1;
                                        vmblock[0] = 1;
    	    		}	
    	    	}).start();}
    	    		...
    	    		new Thread(new Runnable(){
    	    			...
    	    			public void run(){
    	    				...
    	    				instances[1] = new DataCenterInstance("Savior", "http://"+newvm);
                                        System.out.println("now 2nd vm is " + instances[1].getUrl());
    	    				// reset vmstatus and vmblock to 1
                            vmstatus[1] = 1;
                                        vmblock[1] = 1;
    	    		}	
    	    		...
    	    		new Thread(new Runnable(){
    	    			...
    	    			public void run(){
    	    				...
    	    				instances[2] = new DataCenterInstance("Savior", "http://"+newvm);
                                        System.out.println("now 3rd vm is " + instances[2].getUrl());
                                        // reset vmstatus and vmblock to 1
    	    				vmstatus[2] = 1;
                                        vmblock[2] = 1;
    	    		}	
            ...
            // if cpucheckornot arrives 5, it means now its time to check the utilization
            else{
                // give an information showing that enter the check of cpu
                System.out.println("enter check of cpu");
                // only if the 3 data center are all accessable do we make the check, otherwise
                // it will dramatically reduce the rps, because the check of cpu occupies time and resource
            	if(vmblock[0] ==1 && vmblock[1] ==1 && vmblock[2] ==1){
                    /**
                     * check the cpu utilization of 3 data center and print them out
                     */
            		CPUutilization[0] = checkRPS(instances[0].getUrl());
                        System.out.println("cpu 0" + CPUutilization[0]);
    				CPUutilization[1] = checkRPS(instances[1].getUrl());
                        System.out.println("cpu 1" + CPUutilization[1]);
    				CPUutilization[2] = checkRPS(instances[2].getUrl());
                        System.out.println("cpu 2" + CPUutilization[2]);
                    /**
                    ...
                    /**
                     * find the data center with minimum utilization and parse request to this data center
                     * and jump out of for loop
                     */
                    int i = 0;
    				for( i = 0;i <3;i++){
